...
switch(st_menu_level)
{
	...
	case 1:
	for(var c = 0; c < array_length(st_controls); c++)
	{
		var _c = c_white;
		var _binding = [input_binding_get_name(input_binding_get("up")), input_binding_get_name(input_binding_get("down")), input_binding_get_name(input_binding_get("left")), input_binding_get_name(input_binding_get("right")), input_binding_get_name(input_binding_get("confirm")), input_binding_get_name(input_binding_get("cancel")), input_binding_get_name(input_binding_get("menu")) ];
		if st_control_pos == c { _c = c_yellow; }
		draw_text_color(vx + st_op_x + st_border, vy + st_op_y + st_border + (st_op_space*c), st_controls[c], _c, _c, _c, _c, 1)
		for(var b = 0; b < array_length(_binding); b++)
		{
			draw_text(vx + st_op_x + st_border + 65, vy + st_op_y + st_border + (st_op_space*b), _binding[b])
		}
	}
	break;
	case 2:
	draw_text(vx + st_border + 95, vy + st_border + 20, "Select a style for the UI");
